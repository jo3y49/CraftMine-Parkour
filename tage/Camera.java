...
public class Camera
{
	...
	public void Pitch(Float elapsTime){
		Vector3f rightVector = this.getV(); 
		Vector3f upVector = this.getU(); 
		Vector3f fwdVector = this.getN(); 
		...
		this.setV(rightVector); 
		this.setN(fwdVector); 
	}

	public void Yaw(Float elapsTime){
		Vector3f rightVector = this.getU(); 
		Vector3f upVector = this.getV(); 
		Vector3f fwdVector = this.getN(); 
		rightVector.rotateAxis(elapsTime, upVector.x(), upVector.y(), upVector.z()); 
		fwdVector.rotateAxis(elapsTime, upVector.x(), upVector.y(), upVector.z()); 
	...
	public void nMove(Float elapsTime){
		Vector3f oldPosition = this.getLocation();
		Vector4f fwdDirection = new Vector4f(this.getN(), 1f);
